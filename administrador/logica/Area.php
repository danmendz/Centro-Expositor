...
class Area extends Modelo {
	...
	function recuperaInfo($id) {
	    ...
	    LEFT JOIN persona p ON p.id = e.id_persona
	...
	function insertaRegistro() {
		...
		$this->consulta =
		"insert into $this->tabla (nombre, capacidad)".
		"values (".
		"'$this->nombre',".
	    "'$this->capacidad');";
	...
	function asignarArea($idEvento){
	    ...
	    $this->consulta = "SELECT id FROM $this->tabla WHERE id_evento IS NULL";
	    // SELECT id FROM $this->tabla WHERE id_evento IS NULL LIMIT 1
